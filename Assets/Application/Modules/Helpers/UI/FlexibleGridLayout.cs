...
public class FlexibleGridLayout : LayoutGroup
{
    ...
    [Header("GRID SETTING")] public FitType fitType = FitType.Uniform; // The type of fitting
    ...
    [Header("SORTING SETTINGS")] public SortEnum fillFirst = SortEnum.Rows; // Choose what shall be filled first
    ...
    public override void CalculateLayoutInputHorizontal()
    {
        ...
        if (fitType == FitType.DynamicAutoFit)
        {
            fitX = false;
            fitY = false;
            if (fillFirst == SortEnum.Columns)
            {
                // Use fixed cellSize.y, spacing.y, and calculate how many rows fit
                rows = Mathf.Max(1, Mathf.FloorToInt((parentHeight + spacing.y) / (cellSize.y + spacing.y)));
                columns = Mathf.CeilToInt((float)rectChildren.Count / rows);
            }
            else
            {
                // Use fixed cellSize.x, spacing.x, and calculate how many columns fit
                columns = Mathf.Max(1, Mathf.FloorToInt((parentWidth + spacing.x) / (cellSize.x + spacing.x)));
        ...
        // Update the preferred height of the layout so ContentSizeFitter works
        float totalHeight = padding.top + padding.bottom + (cellSize.y * rows) + (spacing.y * (rows - 1));
    ...
    private void SortAndPositionChildren()
    {
        ...
        var sortedChildren = new List<RectTransform>(rectChildren.Count);
        ...
        for (int i = 0; i < sortedChildren.Count; i++)
        {
            ...
            if (childAlignment == TextAnchor.MiddleCenter || childAlignment == TextAnchor.MiddleLeft ||
                childAlignment == TextAnchor.MiddleRight)
            {
                // Vertical centering adjustment with padding respected
                yPos = padding.top + (rectTransform.rect.height - padding.top - padding.bottom -
                                      (rows * cellSize.y + (rows - 1) * spacing.y)) / 2
                                   + (cellSize.y + spacing.y) * rowCount;
            }
            else if (childAlignment == TextAnchor.LowerCenter || childAlignment == TextAnchor.LowerLeft ||
            ...
            if (childAlignment == TextAnchor.MiddleCenter || childAlignment == TextAnchor.UpperCenter ||
                childAlignment == TextAnchor.LowerCenter)
            {
                // Horizontal centering adjustment with padding respected
                xPos = padding.left + (rectTransform.rect.width - padding.left - padding.right -
                                       (columns * cellSize.x + (columns - 1) * spacing.x)) / 2
                                    + (cellSize.x + spacing.x) * columnCount;
            }
            else if (childAlignment == TextAnchor.MiddleRight || childAlignment == TextAnchor.UpperRight ||
                     childAlignment == TextAnchor.LowerRight)
            {

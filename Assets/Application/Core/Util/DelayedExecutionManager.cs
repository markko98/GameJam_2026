...
public class DelayedExecutionManager
{
    ...
    private void addTicket(DelayedExecutionTicket ticket, int fromIndex, int toIndex)
    {
        try
        {
            ...
            var anchor = fromIndex + (int)((float)(toIndex - fromIndex) / 2);

            if (ticket.executionTime > actionList[anchor].executionTime)
            {
                if (anchor == fromIndex || ticket.executionTime <= actionList[anchor + 1].executionTime)
                {
                    //lucky us
                    actionList.Insert(anchor + 1, ticket);
                    return;
                }
                addTicket(ticket, anchor + 1, toIndex);
            ...
            else
            {
                if (anchor == fromIndex)
                {
                    actionList.Insert(anchor, ticket);
                    return;
                }
